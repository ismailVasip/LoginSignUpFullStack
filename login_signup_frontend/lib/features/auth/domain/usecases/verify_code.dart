import 'package:dartz/dartz.dart';
import 'package:login_signup_frontend/core/usecase/usecase.dart';
import 'package:login_signup_frontend/features/auth/data/models/verify_code_params.dart';
import 'package:login_signup_frontend/features/auth/domain/repositories/auth.dart';
import 'package:login_signup_frontend/service_locator.dart';

class VerifyCodeUseCase implements Usecase<Either,VerifyCodeParams>{
  @override
  Future<Either> call({VerifyCodeParams? param}) async{
    return await serviceLocator<AuthRepository>().verifyCode(param!);
      
  }
}